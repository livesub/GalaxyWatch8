...
⚠️ 문제 생기면 이 태그로 원복(최신 안정 기준점, 2026-08-31): 태그 dev-final-stable-v7 (git reset --hard dev-final-stable-v7)
※ v7 = 하단 알약 재설계(라벨삭제/아이콘확대/폰트고정) + 날씨아이콘 계절버그 수정 + 소스정리 반영 시점
※ dev-final-stable-v6 태그는 이 시점 이전의 다른 커밋(f197281, 달의 위상 구현)을 가리키는 기존 태그라 재사용하지 않고 v7로 새로 생성함
※ 이전 기준점(v5, 커밋 10bb50e)은 폐기 — v5~v7 사이 커밋은 필요시 git log로 직접 확인
...
7. **달의 위상 이미지 2종**(`moon_qgf.png` 밝은/보름 계열, `moon_nc.png` 어두운/초승 계열, 각 480x480 원본 → 실사용 50x50 리사이즈, 투명배경 PNG) — 재미나이 생성 후 크로마키 크롭 처리 완료, `watchface/src/main/res/drawable/`에 위치
...
4. 우상단: 달의 위상 표시 구현 완료(구 SLOT_SUNSET 빈 공간 활용) — `MOON_PHASE_TYPE`(0~7 정수) 기반 8단계 Condition 분기, 이미지 2종(`moon_qgf.png` 밝은/보름 계열, `moon_nc.png` 어두운/초승 계열) + 사각형 슬라이딩 마스크(같은 Group 내 renderMode SOURCE/MASK 조합)로 터미네이터 근사 구현(정확한 타원 대신 직선 절단, 9-1 참조). 순수 날짜 계산이라 위치(GPS) 의존 없음. 텍스트 멘트(초승달 등)는 미사용 확정
...
13. 하단 알약1: 워치 배터리(%) — **독립된 타원형 3개**(하나로 합친 바 아님, 최종 원복 확정), 타원 내부는 **아이콘+값만**(2026-08-31 확정: 상단 타이틀 텍스트 "워치 배터리"/"걸음 수"/"심박수" 3개 전부 삭제 — 라벨 없이 아이콘+값 1줄 구성으로 변경, 5-5 참조). 타원 배경은 반투명이되 충분히 진하게(주/야간 배경 모두에서 타원 경계가 보여야 함, 얇은 밝은 테두리 1px 병행), 텍스트는 밝은 민트 계열(흰색·요일색·온도색과 겹치지 않는 미사용 색상)
    ...
    - **아이콘 크기 확대 + 가독성 개선(2026-08-31 확정)**: 라벨 삭제로 생긴 여유 활용, 아이콘 세로형/확대, 아이콘-텍스트 간격 5px 고정, 세부 크기/좌표는 5-5 참조. 배터리 아이콘은 기존 가로형(20x14)에서 세로로 서있는 형태(16x24)로 신규 제작(`icon_battery.png` 교체 완료, 재미나이 생성 후 가짜 체커무늬 배경 제거+실제 alpha 투명 처리해서 적용). 걸음수/심박 아이콘은 크기 박스만 확대 적용, 실제 이미지 파일 교체는 보류 중(기존 파일 그대로 리사이즈 표시)
...
| 달의 위상 Group | 285 | 58 | 82 | 57 | renderMode SOURCE/MASK 구조, 내부 아이콘 로컬좌표 x=16,y=3,w=50,h=50 |
...
### 5-5. 하단 알약 3개 내부 아이콘/값 절대좌표 (2026-08-31 재확정, watchface.xml 실측 — 라벨 삭제 후 아이콘 확대 반영)
| 알약 | 아이콘(PartImage) x,y,w,h | 값(PartText) x,y,w,h |
|---|---|---|
| 배터리(외곽 65,340,96,44) | 68,350,16,24 (`icon_battery.png` 세로형 교체) | 89,354,68,30, size=24 고정 |
| 걸음수(외곽 171,340,96,44) | 179,348,18,28 (이미지 파일은 기존 그대로, 박스만 확대) | 196,354,57,30(자릿수 분기 4개 공통, 5-4 참조) |
| 심박수(외곽 277,340,96,44) | 285,347,30,30 (이미지 파일은 기존 그대로, 박스만 확대) | 320,347,46,30, size=23 고정 |

- 세로 중앙정렬 기준: pill 중심 y=362(=340+44/2) 기준 `아이콘y = 362 - 아이콘height/2`로 계산할 것 — 과거 배터리(y=357)·걸음수(y=355) 값은 이 공식에서 7px 벗어나 하단으로 치우치는 버그였음(2026-08-31 수정 완료), 새 아이콘 추가/교체 시 반드시 이 공식으로 재계산
...
- **실기기 가독성 문제(2026-08-30 발견, 미해결)**: 하단 알약 3개(워치배터리/걸음수/심박수) 텍스트가 잘 안 보임 — 다음 세션 우선 처리 대상. 기준 스크린샷은 세션 로컬 보관 중.
...
- **WFF에는 `<Mask>` 요소가 존재하지 않음**: Samsung Watch Face Studio(GUI 도구) 코드랩의 "Mask" 용어는 GUI 전용 추상 개념이며, raw WFF XML엔 대응 태그가 없음. 실제 구현은 같은 `<Group>` 내 자식 요소에 `renderMode="SOURCE"`(원본)/`renderMode="MASK"`(마스크)를 지정하는 방식 — 여러 MASK 요소는 합집합(OR)으로만 결합되고 교집합/차집합은 불가능. 달의 위상 구현 시 정확한 타원 터미네이터 대신 사각형 슬라이딩 마스크로 근사(직선 절단, ~50px 크기에서 시각적 차이 미미해 채택).
