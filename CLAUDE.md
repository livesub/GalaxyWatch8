...
7. **달의 위상 이미지 7종**(2026-09-06 기존 2종+마스크 방식 폐기, 이미지 스위칭 방식으로 전면 교체 — 실기기 확인 완료): `moon_01_crescent_waxing.png`~`moon_07_crescent_waning.png`, 각 50x50 투명배경 PNG. 재미나이 3x3 그리드 생성물에서 실제 서로 다른 셀 6장(가는/중간 크기 각 방향)+보름 1장을 그대로 크롭, 워터마크(반짝이) 셀만 인페인팅 제거 후 사용. 방향은 좌/우 일관성 우선 배치(하현/그믐은 소스 부족으로 같은 원본 재사용, 톤만 다르게 구분).
    - **색상 2026-09-08 재조정(핑크/블루 배색 폐기)**: 주/야간 배경(보라~코럴/블루퍼플 계열)에 파스텔 핑크·블루가 묻혀 보이는 문제로, 배경과 대비되는 골드~브론즈 톤으로 전면 교체(HSV 색조 치환, hue≈35~45도). 차는 달(초승/상현/차다)은 밝은 골드로 갈수록 진하게, 지는 달(지다/하현/그믐)은 같은 계열의 어두운 브론즈로 갈수록 더 어둡게, 그믐은 특히 짙은 다크브라운+은은한 발광(사라지기 직전 느낌)으로 차별화. 보름은 크림화이트+발광 효과로 특별하게 강조(발광은 방사형 비네트 처리로 정사각 캔버스 가장자리에서 각지게 안 잘리도록 처리, 2026-09-08 확인). 실기기 8단계 전체 순회 확인 완료.
    `watchface/src/main/res/drawable/`에 위치
...
4. 우상단: 달의 위상 표시 구현 완료(구 SLOT_SUNSET 빈 공간 활용) — `MOON_PHASE_TYPE`(0~7 정수) 기반 8단계 Condition 분기로 이미지 7종을 직접 스위칭(2026-09-06, 날씨 아이콘 32종과 동일 패턴). **기존 이미지 2종+사각형 슬라이딩 마스크(SOURCE/MASK) 근사 방식은 폐기**(TYPE0 신월만 이미지 없음, 나머지 7단계 각각 전용 이미지, 4-1/7 참조) — 마스크 직선절단 문제 자체가 해소됨. 순수 날짜 계산이라 위치(GPS) 의존 없음. 실기기 8단계 전환 확인 완료(2026-09-06).
    - **달 이름 라벨 추가(2026-09-06 확정, 이전 "텍스트 멘트 미사용" 결정 번복)**: 아이콘 왼쪽에 세로 2글자 라벨 표시(오전/오후와 동일한 세로배치 방식), MOON_PHASE_TYPE 0~7 → 신월/초승/상현/차다/보름/지다/하현/그믐 8분기(5-2/5-3 좌표·사이즈 참조). "차다"(Waxing Gibbous)/"지다"(Waning Gibbous)는 공식 명칭이 따로 없어 자체 조어. **색상: 한때 차는/지는 구분용 핑크·블루 분기를 시도했으나 2026-09-08 실기기 확인 후 날씨 코멘트와 동일한 흰색+다크 아웃라인으로 최종 통일**(TYPE별 색상 분기 제거, 위상 구분은 이미지 모양만으로)
...
- 온도: **2026-09-08 3단계 색상 분기 최종 확정(커밋 66bfd2c)** — 영하(0도 미만) `#2979FF`(파랑) / 0~27도(평시) `#66BB6A`(초록, 기존 터콰이즈 `#26C6DA`에서 변경) / 28도 이상(고온) `#FF6D00`(오렌지, 유지) 3단계, 서로 다른 색상 계열로 완전히 구분. 모든 분기 공통으로 흰색 4방향(상하좌우, 대각선 없음) 1px 복제 텍스트 아웃라인 적용(검은/다크 아웃라인 아님, 노안 가독성 확보용 — 실기기 확인 완료). 실기기(31도 오렌지)/에뮬레이터(0도 초록)/임시 하드코딩 테스트(-5도 파랑) 3가지 모두 확인 완료
...
- 달 이름 라벨: **2026-09-08 재확정 — 흰색(#FFFFFF) 단독, 아웃라인 없음**(TYPE별 색상 분기는 폐기했으나, 뒤이어 아웃라인도 폰트 크기 23 BOLD에서 검정 테두리가 과하게 두꺼워 보여(에뮬레이터 확인) 제거 — 8방향 복제 텍스트 구조 삭제, PartText 1개(흰색)만 유지)
