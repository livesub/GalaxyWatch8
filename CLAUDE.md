...
⚠️ 문제 생기면 이 커밋/태그로 원복(최신 안정 기준점, 2026-08-31): 커밋 8d30b2b / 태그 dev-final-stable-v7 (git reset --hard dev-final-stable-v7)
※ v7 = 하단 알약 재설계(라벨삭제/아이콘확대/폰트고정) + 날씨아이콘 계절버그 수정 + 소스정리 반영 시점
※ v6(커밋 f197281)은 이미 존재하던 다른 태그라 그대로 두고 v7로 새로 생성(충돌 회피) — 이전 기준점(v5, 커밋 10bb50e)은 폐기
...
2-1. **하단 알약 아이콘 3종**(`icon_battery.png`, `icon_heart.png`, `icon_steps.png`) — watchface.xml에서 실제 참조되는 별도 이미지 파일(코드로 직접 그리는 벡터가 아님), 워치배터리/심박수/걸음수 알약 좌측에 배치. **2026-09-06: 3종 전부 신규 캐릭터 이미지로 교체**(재미나이 생성 원본 → 배경 제거+trim+26x26 컨테인 리사이즈), 배터리는 반투명 캡/얼굴 영역 alpha 보정판 적용(5-5 참조)
3. **배경 이미지 2종**(day/night, 각 1장) — 6-1-1의 그라디언트 톤 기준으로 생성(제미나이 등), 사각형으로 나와도 반드시 원형 크롭 후 사용
4. **폰트 파일 2종**: `notosanskr_bold.ttf`, `notosanskr_regular.ttf`(SIL OFL) — 처음부터 서브셋 버전으로 준비 권장. **2026-09-06 기준 89자 세트(기존 76자 + 달 이름 라벨용 12자 "신초승상현차다름지하그믐" 추가)**: 숫자 0-9, 콜론, 마침표, 가운뎃점(·), 도(°), 퍼센트(%), "오전""오후", 요일 한글 7개, "강수확률""워치배터리""걸음수""심박수""정보 없음""날씨 데이터 로드 실패, 실기기에서 확인 필요", `docs/weather_icons.md` 날씨 코멘트 문구 전부, "바람개비 워치8 클래식", 달 이름 라벨 8종(신월/초승/상현/차다/보름/지다/하현/그믐) — 처음부터 이 89자 문자셋으로 서브셋하면 재작업 불필요. 서브셋 도구는 harfbuzz 기반 방식 사용(시스템 NotoSansKR-VF.ttf Bold 인스턴스에서 재서브셋, 2026-09-06 확인)
...
7. **달의 위상 이미지 7종**(2026-09-06 기존 2종+마스크 방식 폐기, 이미지 스위칭 방식으로 전면 교체 — 실기기 확인 완료): `moon_01_crescent_waxing.png`~`moon_07_crescent_waning.png`, 각 50x50 투명배경 PNG. 재미나이 3x3 그리드 생성물에서 실제 서로 다른 셀 6장(가는/중간 크기 각 방향)+보름 1장을 그대로 크롭, 워터마크(반짝이) 셀만 인페인팅 제거 후 사용. 색상은 HSV 색조 치환으로 후처리(차는 달 계열 `#F06292` 핑크 톤, 지는 달 계열은 원본 파스텔블루 유지, 방향 일관성 우선 배치 — 하현/그믐은 소스 부족으로 같은 원본 재사용, 명도만 다르게 구분). `watchface/src/main/res/drawable/`에 위치
...
1. 알림 개수: 커스텀 점(dot) 완전 삭제 확정(시스템이 알림 아이콘을 자동 표시해 중복이라 제거) — 좌측 빈공간(중단 좌측 콤플리케이션 바깥, 원 세로 중앙 높이)에 숫자만 표시, 읽지 않은 알림 있을 때만 표시(없으면 숨김), 시스템 알림 아이콘과 약 10px 간격 확보
...
4. 우상단: 달의 위상 표시 구현 완료(구 SLOT_SUNSET 빈 공간 활용) — `MOON_PHASE_TYPE`(0~7 정수) 기반 8단계 Condition 분기로 이미지 7종을 직접 스위칭(2026-09-06, 날씨 아이콘 32종과 동일 패턴). **기존 이미지 2종+사각형 슬라이딩 마스크(SOURCE/MASK) 근사 방식은 폐기**(TYPE0 신월만 이미지 없음, 나머지 7단계 각각 전용 이미지, 4-1/7 참조) — 마스크 직선절단 문제 자체가 해소됨. 순수 날짜 계산이라 위치(GPS) 의존 없음. 실기기 8단계 전환 확인 완료(2026-09-06).
    - **달 이름 라벨 추가(2026-09-06 확정, 이전 "텍스트 멘트 미사용" 결정 번복)**: 아이콘 왼쪽에 세로 2글자 라벨 표시(오전/오후와 동일한 세로배치 방식), MOON_PHASE_TYPE 0~7 → 신월/초승/상현/차다/보름/지다/하현/그믐 8분기(5-2/5-3 좌표·사이즈 참조). "차다"(Waxing Gibbous)/"지다"(Waning Gibbous)는 공식 명칭이 따로 없어 자체 조어. 스타일은 일출일몰시각·날씨코멘트와 동일(notosanskr_bold/BOLD, 아웃라인 없음)
    - **차는/지는 달 라벨 색상 구분(2026-09-06 확정, 사용자가 위상 구분 어려움 호소)**: 사용자가 초승달 모양만으로 차는 달/지는 달 구분이 어렵다고 하여 라벨 색상으로 구분 추가 — 차는 달(초승/상현/차다, TYPE 1~3) `#F06292`(핑크, 요일 '금' 색상 재사용), 지는 달(지다/하현/그믐, TYPE 5~7) `#4FC3F7`(스카이블루, 요일 '월' 색상 재사용), 신월/보름(TYPE 0,4)은 기존대로 흰색 유지
...
9. 중단 우측 원형 콤플리케이션: 앱 바로가기 슬롯, 용도는 음성녹음(위와 동일하게 사전 지정 불가), 사용자가 직접 지정
   ...
   - **미설정(EMPTY) 상태 표시(2026-08-30 재정정)**: App Shortcut 3개 슬롯(TOP_LEFT/TOP_RIGHT/MID_RIGHT)은 미설정 시 원형 테두리 + "+" 아이콘을 watchface.xml 자체에서 그려서 평소 화면에도 항상 보이게 처리(실기기 확인 결과 이게 없으면 아무것도 안 그려지는 버그 확인). SLOT_MID_LEFT(휴대폰배터리)는 자체 EMPTY 분기가 정상 렌더링되어 이 플레이스홀더 제거. 콤플리케이션 편집(꾸미기) 화면에서 시스템이 자동으로 그려주는 "+"는 이것과 별개(그건 그대로 유지, 구현 불필요) — 지금 말하는 건 편집화면이 아닌 **평소 시계 화면**에 뜨는 자체 제작 원형+"+" 플레이스홀더. 실제 값/앱이 설정되면 콤플리케이션 콘텐츠가 그 위에 렌더링되어 "+"를 덮음(단, SLOT_MID_LEFT는 자체 EMPTY 분기가 정상 렌더링되어 이 플레이스홀더 제거 확정 — App Shortcut 3개 슬롯(TOP_LEFT/TOP_RIGHT/MID_RIGHT)은 미설정 시 아무것도 안 그려지는 실기기 버그 확인되어 플레이스홀더 유지 필수, 2026-08-30)
...
13. 하단 알약1: 워치 배터리(%) — **독립된 타원형 3개**(하나로 합친 바 아님, 최종 원복 확정), 타원 내부는 **아이콘+값만**(2026-08-31 확정: 상단 타이틀 텍스트 "워치 배터리"/"걸음 수"/"심박수" 3개 전부 삭제 — 라벨 없이 아이콘+값 1줄 구성으로 변경, 5-5 참조). 타원 배경은 반투명이되 충분히 진하게(주/야간 배경 모두에서 타원 경계가 보여야 함, 얇은 밝은 테두리 1px 병행), 텍스트는 밝은 민트 계열(흰색·요일색·온도색과 겹치지 않는 미사용 색상)
    ...
    - **아이콘 크기 확대 + 가독성 개선(2026-08-31 확정, 2026-09-06 재개정)**: 라벨 삭제로 생긴 여유 활용, 아이콘 세로형/확대, 아이콘-텍스트 간격 5px 고정. **2026-09-06: 3개 아이콘 전부 26x26 정사각 박스로 크기 통일**(이전 배터리 16x24/걸음수 18x28/심박 30x30 개별크기 폐기), 좌측여백 6px 고정 확정 — 알약 stadium 곡률 안전범위 계산상(half-height ≤ √(22²-(22-margin)²)) 26x26+여백6px가 여유(15.1) 있게 안전, **28x28+여백4px는 곡률 침범(아이콘 상하단이 알약 밖으로 튀어나옴) 확인되어 폐기**. 배터리/걸음수/심박수 이미지 파일 3종 전부 신규 캐릭터 이미지로 교체(재미나이 생성 원본 → rembg 배경 제거+trim+26x26 컨테인 리사이즈), 배터리는 반투명 캡/얼굴 영역이 원본 alpha가 낮아 축소 시 형체가 거의 안 보이는 문제 있어 해당 영역만 alpha·색상 보정(불투명 처리)한 버전 적용. 세부 좌표는 5-5 참조
...
|---|---|---|---|---|---|
| 알림 개수 | 20 | 207 | 30 | 20 | 점 삭제, 숫자만 표시, 알림 있을 때만, 가운데 정렬 |
...
| 강수확률 값 | 374 | 235 | 60 | 22 | "NN%" 형식, 가운데 정렬, 색상 `#4FC3F7`+다크아웃라인, `isAutoSize="TRUE" minSize="9"` 적용, 필드 WEATHER.DAYS.0.CHANCE_OF_PRECIPITATION |
...
| 달 이름 라벨 | 271 | 61 | 30 | 50 | 세로 2글자(오전/오후 방식), MOON_PHASE_TYPE 0~7→신월/초승/상현/차다/보름/지다/하현/그믐 |
...
| 달 이름 라벨 | 23 | notosanskr_bold/BOLD, 아웃라인 없음, 색상은 TYPE별 분기(6-1 참조) |
...
- 달 이름 라벨(차는/지는 구분): 차는 달(초승/상현/차다) `#F06292`(요일 '금' 재사용), 지는 달(지다/하현/그믐) `#4FC3F7`(요일 '월' 재사용), 신월/보름은 흰색 — 아웃라인 없음
...
- 안읽은 알림 인디케이터: 커스텀 점(dot) 삭제 확정(시스템 자동 아이콘과 중복이라 제거), 알림 개수 숫자만 알림 있을 때 표시
...
- **에뮬레이터는 위치 정보 부재로 `WEATHER.IS_DAY`가 항상 주간(true)으로 고정됨(2026-09-06 확인)** — 야간 배경/야간 조건부 요소(일출일몰 라벨, 날씨 아이콘 night 계열 등) 검증은 에뮬레이터로 불가, 실기기가 실제 밤 시간이거나 워치 날짜를 직접 조작해야 확인 가능
...
- 터치 타겟 확장(2026-08-30 확정): 4개 슬롯 전부 `BoundingOval` 0,0,52,52 → -6,-6,64,64로 확장(터치 인식 개선용, 표시 크기 52x52는 변경 없음)
...
- **콤플리케이션 편집(꾸미기) 화면 번호 배지 순서 버그(2026-08-30 발견, 미해결)**: 실기기에서 번호 배지 순서가 슬롯 위치와 안 맞음(하단 좌측이 3번이어야 하는데 다른 슬롯이 바뀜). XML 선언 순서는 정상 확인됨, ADB로 편집화면 UI 접근 불가, 에뮬레이터는 번호 배지 UI 자체가 없음(삼성 One UI Watch 전용 스킨, 구글 스톡 Wear OS 에뮬레이터엔 미존재)이라 재현 불가 확인됨. **추가 확인(2026-08-31)**: 워치 자체(온디바이스 꾸미기)에서 슬롯 지정은 정상 동작 — 문제는 스마트폰 Galaxy Wearable 앱 쪽 편집 화면에서만 발생하는 것으로 좁혀짐. 좀 더 관찰 후 재진행(보류 유지)
