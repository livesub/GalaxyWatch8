...
- 포맷: Watch Face Format (WFF) v2, watchface.xml 선언형 (v5 상향 시도했으나 실기기/에뮬레이터에서 패키지 자체 거부(FavoriteOperationException) 확인 → v2로 최종 확정. isAutoSize/verticalAlign은 v2 XSD상 미지원 리포트 뜨지만 실제 렌더 정상 동작 확인되어 그대로 유지)
...
13. 하단 알약1: 워치 배터리(%) — **독립된 타원형 3개**(하나로 합친 바 아님, 최종 원복 확정), 타원 내부에 타이틀(예: "워치 배터리")+아이콘+값 2줄 구성. 타원 배경은 반투명이되 충분히 진하게(주/야간 배경 모두에서 타원 경계가 보여야 함, 얇은 밝은 테두리 1px 병행), 텍스트는 밝은 민트 계열(흰색·요일색·온도색과 겹치지 않는 미사용 색상)
    ...
    - **박스 폭 차등 확정**: 각 알약의 예상 최대 자릿수에 맞춰 폭 고정(심박수=소 76px, 배터리=중 96px, 걸음수=대 124px) — 동일폭으로 두면 심박수처럼 자릿수 적은 항목은 낭비, 걸음수처럼 많은 항목은 비좁아짐 방지
    - **걸음수(자릿수 변동 큰 항목) 값 폰트**: 고정폭 대비 자릿수에 따라 축소뿐 아니라 확대도 하는 양방향 오토핏(예: 1~2자리↑크게, 5자리↓작게) — 값이 짧을 때 박스 가운데 빈 공간 생기는 것 방지. 배터리/심박수는 자릿수 변동폭이 작아(배터리 0~100%, 심박수 2~3자리 고정) 폰트 고정, 오토핏 불필요
14. 하단 알약2: 삼성 헬스 걸음 수 — 알약1과 동일 스타일(독립 타원, 타이틀 내장), 폭은 위 박스 폭 차등 규칙에 따라 대(large)
15. 하단 알약3: 심박수(HEART_RATE) 단독 확정(스트레스 미구현) — 알약1/2와 동일 스타일, 폭은 소(small)
...
| 초 | 311 | 274 | 49 | 50 | |
| 알약(워치배터리) | 65 | 340 | 96 | 44 | 폭 중(md) |
| 알약(걸음수) | 167 | 340 | 124 | 44 | 폭 대(lg) |
| 알약(심박수) | 297 | 340 | 76 | 44 | 폭 소(sm) |
...
- 런처 아이콘(워치페이스 목록 썸네일): 미확보 — 별도 준비 필요
...
- 런처 아이콘(워치페이스 목록 썸네일) — 리소스/디자인 전체 완성 후 마지막에 제작
